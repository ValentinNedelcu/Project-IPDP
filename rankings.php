	...
	<title>Rankings</title>
		...
		<div class="row">
			<div class="leftcolumn">
				...
				$query = "SELECT id ,nume, tara FROM campionat";
				$stmt = $connect->prepare($query);
				$stmt->execute();
				$rows = $stmt->fetchAll();
				foreach ($rows as $a) {
					echo '<br><h3>'.$a['nume'].'----'.$a['tara'].'</h3>';
					$stocare = $a['id'];

					$query1 = "SELECT clasare, echipe.nume as nume, goluriMarcate, goluriPrimite, golaveraj ,numarPuncte FROM clasament, echipe, campionat WHERE clasament.idEchipa = echipe.id and clasament.idCampionat = campionat.id and campionat.id='$stocare'";
					$stmt1 = $connect->prepare($query1);
					$stmt1->execute();
					$rows1 = $stmt1->fetchAll();

					echo "<table><tr><th>Loc</th><th>Nume</th><th>GM</th><th>GP</th><th>Golaveraj</th><th>Total puncte</th></tr>";
					foreach($rows1 as $b){
						echo "
							<tr>
								<td>".$b['clasare']."</td>
								<td>".$b['nume']."</td>
								<td>".$b['goluriMarcate']."</td>
								<td>".$b['goluriPrimite']."</td>
								<td>".$b['golaveraj']."</td>
								<td>".$b['numarPuncte']."</td>
							</tr>";
					 }
